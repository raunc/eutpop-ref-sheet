...
\newlength{\fhDefHre} \setlength\fhDefHre{28.5\baselineskip}
...
\begin{dynamiccontents*}{frameDowRestrictions}\begin{eubox}{\fhDowRestrictions}
	% \begin{multicols}{2}
		...
		\begin{itemize}
			...
			\item Events may specify other restrictions
		\end{itemize}
	% \end{multicols}
...
\begin{dynamiccontents*}{frameHreIntWars}\begin{eubox}{\fhHreIntWars}
	% \begin{multicols}{2}
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item Apply normal penalties for missing CB
...
\begin{dynamiccontents*}{frameCallToArms}\begin{eubox}{\fhCallToArms}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item \hspacezero\zsavepos{arrowReceiveCtAStart}Calling a PR Ally has no cost
...
\begin{dynamiccontents*}{frameReceivingCta}\begin{eubox}{\fhReceivingCta}
	\begin{multicols}{2}
		\subsubsection*{Receiving a CtA \normal{(p.~32-33)}\zsavepos{arrowReceiveCtAEnd}}
		\begin{itemize}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\begin{itemize}
					...
					\begin{itemize}
						% TODO: This is not explicitly written in rules. Instead, it was clarified on Aegir Games Discord. Add reference once it is added to FAQ/Errata
...
\begin{enumerate}
	\item \hspacezero\zsavepos{arrowDowRestrictionsStart}Pick target Realm(s), place War tokens
...
\begin{dynamiccontents*}{frameArrowsPageOne}
	...
	\newlength{\dowRestrictionsStartY} \setlength{\dowRestrictionsStartY}{\calc{\zposy{arrowDowRestrictionsStart}sp + \arrowAdjustYAfterSubsub}}
	\newlength{\dowRestrictionsStartX} \setlength{\dowRestrictionsStartX}{\calc{\zposx{arrowDowRestrictionsStart}sp + \arrowAdjustXBeforeList}}
	...
	\drawArrow{(\marriageStartX, \marriageStartY) -- (\marriageTurnX,\marriageEndOneY) -- (\marriageEndOneX,\marriageEndOneY)}
	\drawArrow{(\marriageStartX, \marriageStartY) -- (\marriageTurnX,\marriageEndOneY) -- (\marriageTurnX,\marriageEndTwoY) -- (\marriageEndTwoX,\marriageEndTwoY)}
	...
	\newlength{\receiveCtaStartX} \setlength{\receiveCtaStartX}{\calc{\zposx{arrowReceiveCtAStart}sp + \arrowAdjustXBeforeList}}
	\newlength{\receiveCtaEndY} \setlength{\receiveCtaEndY}{\calc{\zposy{arrowReceiveCtAEnd}sp + \arrowAdjustYAfterSubsub}}
	\newlength{\receiveCtaEndX} \setlength{\receiveCtaEndX}{\zposx{arrowReceiveCtAEnd}sp}
	\newlength{\receiveCtaTurnX} \setlength{\receiveCtaTurnX}{\arrowXVerticalMidRight}
...
\begin{enumerate}
	...
	\begin{itemize}
		...
		\item Pay +1\milpower per Sieging Unit after the first
	\end{itemize}
...
\begin{dynamiccontents*}{frameBattleSequence}\begin{eubox}{\fhBattleSequence}
	\begin{multicols}{2}
		...
		\begin{itemize}
			...
			\item Remove remaining NPR units
		\end{itemize}
	\end{multicols}
\end{eubox}\end{dynamiccontents*}
