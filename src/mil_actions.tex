...
\begin{dynamiccontents*}{frameCasusBelli}\begin{eubox}{\fhCasusBelli}
	...
	\strong{Conquest (Claim)} -- Have \claim (or \core (p.~21)) in Area where target Lawfully Owns or Controls any Prov., \conj{or} has any Vassals (p.~33)\\
...
\begin{dynamiccontents*}{frameActiveAlly}\begin{eubox}{\fhActiveAlly}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Gain 1\milpower if \ally is Adj. to new Enemy, \conj{and} activated as part of CtA (p.~13)
			\end{itemize}
		{\botrules
...
\begin{dynamiccontents*}{frameReceivingCta}\begin{eubox}{\fhReceivingCta}
	\begin{multicols}{2}
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\begin{itemize}
					...
					\begin{itemize}
						\botrule{\item Bot sends \emph{Def. CtA}s to all valid \allies Adj. to the Aggressor (p.~4)}
					\end{itemize}
				\end{itemize}
			\end{itemize}
		...
		\begin{itemize}
			...
			\end{itemize}
			\item Place Truce tokens, \conj{unless} former Ally is PR who has CB token on your Capital
		\end{itemize}
	\end{multicols}
\end{eubox}\end{dynamiccontents*}
...
\begin{enumerate}
	...
	\end{itemize}
	\item \action{Calls to Arms} (in listed order, a to d)\zsavepos{arrowCtAStart}
	\begin{enumerate}[label=\alph*.]
		...
		\begin{itemize}
			\botrule{\item Bot sends \emph{Def. CtA}s to all valid \allies Adjacent to the Aggressor (p.~4)}
		\end{itemize}
	\end{enumerate}
...
\begin{itemize}
	...
	\item May \strong{fight Pirates} in Trade Node Adjacent to a Sea Zone where Ships ended \action{Naval Activation} (p.~25, 28)
	\item Activated Light Ships may occupy vacant \strong{Trade Protection} slots in non-Hostile destinations (p.~25)
\end{itemize}
...
\begin{itemize}
	...
	\end{itemize}
	\item \hspacezero\zsavepos{arrowNavalBridgeTwoStart}May move (after Nav. Battle if any) a Land Unit/Army via \strong{Naval Bridge} if (p.~26)
	\begin{itemize}
		\item It includes destination Sea Zone, \conj{and}
...
\begin{dynamiccontents*}{frameNprWarfare}\begin{eubox}{\fhNprWarfare}
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item Extra Units defending HRE Members in HRE Areas if \strong{Emp. is NPR} (p.~45)
			\begin{itemize}
...
\begin{dynamiccontents*}{frameBattleSequence}\begin{eubox}{\fhBattleSequence}
	\begin{multicols}{2}
		...
		\begin{itemize}
			...
			\item Attacker is Active PR or who Decl. War
			\botrule{\item If Bot is Attacker or Main Defender, follow Bot Action charts on p.~16 (p.~5)}
			{\botrules
			\item If Bot fights alongside another PR (p.~6)
			\begin{itemize}
				\item Bot's \ship counts as 3 NPR \ships
			\end{itemize}
			}
		\end{itemize}
		\smallheading{1. Battle Preparations}
		\begin{itemize}
			\item \strong{Emperor} may add all Imperial \manpower (p.~44)
			\begin{itemize}
			...
			\end{itemize}
			\item If 2+ PRs defend, pick \strong{Main Defender}:
			\begin{enumerate}
				\item \botrule{Humans before Bots (p.~5)}
				\item PR with the most Units
				\item PR defen. who last took a Turn decides
			\end{enumerate}
			% Rules actually say Active Player instead of Attacker, but this seems not to be the intent in all cases. See: https://discord.com/channels/567337533462151178/1120978517056827464/1120978517056827464
			\item Only Attacker and Main Defender may
			\begin{itemize}
		...
		\begin{itemize}
			\item If \strong{Land Battle}, default 3\infantry Dice
			\begin{itemize}
				\item 3\infantry or 3\cavalry for Muslim PRs (p.~38)
			\end{itemize}
			\item If \strong{Naval Battle}, default 3\artillery Dice
		...
		\begin{itemize}
			\item If \strong{multiple factions} on same side, then
			\begin{itemize}
		...
		\begin{itemize}
			...
			\item Resolve separately for each \strong{PR}
			\begin{itemize}
			...
			\item \strong{Retreat destinations}
			\begin{itemize}
