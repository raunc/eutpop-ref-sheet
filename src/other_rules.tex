...
\begin{itemize}
	\item \strong{Adjacency}:
	\begin{itemize}
		\item \strong{Prov.}/\strong{Unit}/\strong{token} is Adj. to its Area, bordering Areas, Prov. in those Areas
		\item \strong{Port} is also Adj. to its Sea Zones, Ports facing same Sea Z., Areas of such Ports
		\item \strong{Areas}/\strong{Sea Zones} are Adj. if they share a border. \strong{Areas} are also Adj. to Areas with Ports facing the same Sea Zone
		\item \strong{Realm} is Adjacent to anything its Own Provinces are Adjacent to
	\end{itemize}
	\item \strong{Deployed}: Unit on board or in Army/Fleet
	...
	\item \strong{Occupied Province} has Occ. token \conj{or} 2 \rebeltown/\town/\vassal (Controller on top, Lawful Owner beneath)
	\item \strong{Own Province} has its Realm's flag/\town/\dnpr \conj{and} is non-Occupied
	\item \strong{Realm} consists of Own Provinces and Areas with Own Provinces
	\begin{itemize}
		\item \strong{(N)PR}: (Non-)Player Realm
		\item \strong{Distant R.}: with Dist. Capital
	\end{itemize}
	\item Realm's \strong{Core Prov.} has its flag/\dnpr on Prov. or \core in Area (\core/\dnpr supercede flags)
	\item \strong{Colonial \claim}: \claim on vacant Territory (p.~12)
\end{itemize}

\subsubsection*{Token Limits \rulesref{(p.~2)}}
\begin{itemize}
	\item Generally limited. If nothing in supply, take from anywhere. Exceptions below
	\item \strong{May not be (re)moved}
	\begin{itemize}
...
\subsubsection*{Monarch Power (\monarchpower) \rulesref{(p.~19)}}
\begin{itemize}
	\item No more than 10 \monarchpower of any type
\end{itemize}

\subsubsection*{Characters \rulesref{(p.~19)}}
\begin{itemize}
	\item Max 1 Ruler
...
\subsubsection*{Stability (\stability) \rulesref{(p.~20)}}
\begin{itemize}
...
\begin{itemize}
	\item If a PR cannot pay a mandatory \ducat cost and cannot take a Loan, they
	\begin{itemize}
		\item First pay as much \ducats as they can, then
...
\subsubsection*{Core Provinces (\core) \rulesref{(p.~21)}}
\begin{itemize}
...
\subsubsection*{Influence (\influence) \rulesref{(p.~32)}}
\begin{itemize}
...
\subsubsection*{Marriages (\marriage) \rulesref{(p.~32)}}
\begin{itemize}
...
\subsubsection*{Alliances (\ally) \rulesref{(p.~32)}}
\begin{itemize}
...
\subsubsection*{Vassals (\vassal) \rulesref{(p.~33)}}
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
	\end{itemize}
	\item \strong{\guilsinglright Papal Stat.} and \strong{Emp.} cannot be Vassalized
...
\begin{itemize}
	...
	\item NPR's \strong{Vassals} count as part of Overlord during War and Peace Resolution
	\begin{itemize}
		\item May be ceded in Peace Resolutions
	\end{itemize}
	\item \strong{Released Vassals} (Overlord does not exist, or \vassal was removed) become NPRs
\end{itemize}

\subsubsection*{DNPRs (\dnpr) \rulesref{(p.~35)}}
\begin{itemize}
...
\subsubsection*{\botrules Bot rolls \botrulesref{(p.~4)}}
{\botrules
\begin{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item If Province is Core of PR who has \town/\vassal on it \conj{and} not Occupied
		\begin{itemize}
	...
	\begin{itemize}
		...
		\item PR whose Province is Liberated by Rebels must \strong{place a CB token} on new Owner's Capital and remove any Truce
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item May \action{Change State Religion}
	\end{itemize}
	\item Protestantism
	\begin{itemize}
		\item \action{Conv. Area} and \action{Developm.} cost -1\adminpower
		\item May \action{Change State Religion}
	\end{itemize}
	\item{Orthodoxy}
	\begin{itemize}
...
\subsubsection*{Missions \rulesref{(p.~42)}}
\begin{itemize}
	\item Rewards follow Event guidelines
\end{itemize}

\subsubsection*{Milestones \rulesref{(p.~42-43)}}
\begin{itemize}
...
\subsubsection*{Imperial Authority (\authority) \rulesref{(p.~43)}}
\begin{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item May not be removed to \action{Subjugate}
	\end{itemize}
\end{itemize}
...
\begin{itemize}
	\item If HRE has official Religion, Emp. may only \action{Change State Rel.} to HRE Religion 
...
\begin{itemize}
	...
	\item Triggered when Emp.'s Ruler is replaced or discarded \conj{and} \authority ≤ 4
...
\begin{itemize}
	\item Bot's Covert Actions can be countered with \action{Counterespionage} as normal
	\item Bots never counter Covert Actions 
\end{itemize}

\subsectionbot{Bot Warfare \botrulesref{(p.~4-6)}}
\begin{itemize}
...
\begin{itemize}
	...
	\end{itemize}
	\item If \strong{Invalid target}, choose another:
	\begin{enumerate}
