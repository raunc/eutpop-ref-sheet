...
\newlength{\fhFirstPageLeftCol} \setlength\fhFirstPageLeftCol{\calc{\textheight - \fhNprInvasions - \frameToTextSpacing}} 
\newlength{\fhFirstPageMiddleCol} \setlength\fhFirstPageMiddleCol{\calc{\textheight - \fhNprInvasions - \fhPeaceSeq - 2\frameToTextSpacing}}
\newlength{\fhFirstPageRightCol} \setlength\fhFirstPageRightCol{\calc{\textheight - \fhPeaceSeq - \frameToTextSpacing}}
...
% Text Columns
\usepackage{flowfram}
\newflowframe[1]{\columnwidth}{\fhFirstPageLeftCol}{0mm}{\calc{\fhNprInvasions + \frameToTextSpacing}}[firstPageLeftCol]
\newflowframe[1]{\columnwidth}{\fhFirstPageMiddleCol}{\middleColX}{\calc{\fhNprInvasions + \frameToTextSpacing}}[firstPageMiddleCol]
\newflowframe[1]{\columnwidth}{\fhFirstPageRightCol}{\rightColX}{0mm}[firstPageRightCol]
\newflowframe[2]{\columnwidth}{\fhSecondPage}{0mm}{0mm}[secondPageLeftCol]
\newflowframe[2]{\columnwidth}{\fhSecondPage}{\middleColX}{0mm}[secondPageMiddleCol]
\newflowframe[2]{\columnwidth}{\fhSecondPage}{\rightColX}{0mm}[secondPageRightCol]
...
% Text Frames
\newdynamicframe[1]{\fwTwoCols}{\fhPeaceSeq}{\middleColX}{\calc{\textheight - \fhPeaceSeq}}[framePeaceSeq]
\newdynamicframe[1]{\fwTwoCols}{\fhNprInvasions}{0mm}{0mm}[frameNprInvasions]

\newdynamicframe[2]{\textwidth}{\fhPeaceTerms}{0mm}{\calc{\textheight - \fhPeaceTerms}}[framePeaceTerms]
...
\begin{dynamiccontents*}{frameNprInvasions}\begin{eubox}{\fhNprInvasions}
	\begin{multicols}{2}
		\subsubsection*{\zsavepos{arrowNprInvasionsEnd}NPR Invasions \normal{(p.~36)}}
		\begin{itemize}
			\item Resolve NPRs in alphabetical order
			\item \strong{Invasion takes place if}
			\begin{itemize}
				\item The NPR is not an Active Ally, \conj{and}
				\item The NPR or its Vassals have 1+ Prov. in Areas without Hostile Units, \conj{and}
		...
		\begin{enumerate}[label=\strong{\arabic*.}]
			...
			\begin{itemize}
				\botrule{\item Place Bot Army in the Area (p.~2)}
			\end{itemize}
...
\begin{dynamiccontents*}{framePeaceSeq}\begin{eubox}{\fhPeaceSeq}
	\begin{multicols}{2}
		...
		\begin{itemize}
			\item Treat \strong{Vassals} as part of their Overlord
			\item Making Peace flips respective War token(s)
			\item In each step, resolve Peace in Turn order
			\item On PR's Turn, they resolve all Wars in that step using Terms whose req. are met\zsavepos{arrowPeaceTermsStart}
			\begin{itemize}
				\item 1 Enemy at a time in any order, except normally Active Allies are resolved together with their PR Ally 
				\botrule{\item Bots use Peace Resolution chart (p.~15)}
			\end{itemize}
		\end{itemize}
		...
		\begin{itemize}
			...
			\item Treat \strong{Active Allies} separately
...
\sectionbg{Sequence \normal{\Large{(p.~8-11)}}}
%\framebreak % Hack: without manual frame break subsection heading is not red (this was needed when NPR Invasions text box immediately followed the main heading)
...
% \botrule{\item Bots use Main Turn Structure (p.~7)} % Not very important and no room for this. Uncomment if things are rearranged and it fits again
\end{itemize}
...
\subsubsection*{A. Remove CB tokens \& Truces}
\begin{itemize}
...
\subsubsection*{C. Rebels Siege \conj{or} Move}
\begin{itemize}
...
\subsubsection*{D. Peace Resolutions\zsavepos{arrowPeaceSeqStart}}
\begin{itemize}
	\item While \strong{Def. the HRE} is active (p.~44)
	\begin{itemize}
		\item External Aggressor may \conj{never} resolve Peace separately with NPR Imp. Subj., \conj{unless} Victory over Emp. is achieved
		\item Treat NPR Imp. Subjects as Emp.'s \activeallies
	\end{itemize}
	\item When \strong{Emperor} makes Peace with an External Aggressor (p.~44-45)
	\begin{itemize}
		\item If Emperor won \conj{and} no HRE Prov. is ceded, raise \authority by 1
		\item If Emperor lost, drop \authority by 1
	\end{itemize}
	\item Raise/drop \authority by \# of Areas joining/leaving HRE (p.~45)
	\item PR is the Victor if (p.~29-31)
	\begin{itemize}
		\item They achieved Total/Partial Victory, \conj{or}
		\item Their Enemy Surrendered, \conj{or}
...
\begin{dynamiccontents*}{frameArrowsPageOne}
	...
	% Peace Resolution Sequence arrow
	\newlength{\peaceSeqStartY} \setlength{\peaceSeqStartY}{\calc{\zposy{arrowPeaceSeqStart}sp + \arrowAdjustYAfterSubsub}}
	\newlength{\peaceSeqStartX} \setlength{\peaceSeqStartX}{\calc{\zposx{arrowPeaceSeqStart}sp + \arrowToTextSpacing}}
	\newlength{\peaceSeqEndY} \setlength{\peaceSeqEndY}{\calc{\margin + \textheight - \fhPeaceSeq - .5\boxpadding}}
	\newlength{\peaceSeqEndX} \setlength{\peaceSeqEndX}{\calc{\margin + \textwidth + .5\boxpadding}}
	\newlength{\peaceSeqTurnX} \setlength{\peaceSeqTurnX}{\arrowXVerticalRight}
	\newlength{\peaceSeqTurnY} \setlength{\peaceSeqTurnY}{\calc{\peaceSeqEndY - .5\columnsep}}

	\drawArrow{(\peaceSeqStartX,\peaceSeqStartY) -- (\peaceSeqTurnX,\peaceSeqStartY) -- (\peaceSeqTurnX,\peaceSeqTurnY) -- (\peaceSeqEndX,\peaceSeqEndY)}

	% Peace Terms arrow (continues on the next page)
	...
	\newlength{\peaceTermsTurnX} \setlength{\peaceTermsTurnX}{\arrowXVerticalMidRight}
	\newlength{\peaceTermsTurnY} \setlength{\peaceTermsTurnY}{\calc{\margin + \textheight + .5\columnsep}}

	\drawLine{(\peaceTermsStartX,\peaceTermsStartY) -- (\peaceTermsTurnX,\peaceTermsStartY) -- (\peaceTermsTurnX,\peaceTermsTurnY) -- (\pagewidth,\peaceTermsTurnY)}
	...
	\newlength{\nprEndY} \setlength{\nprEndY}{\calc{\zposy{arrowNprInvasionsEnd}sp + \arrowAdjustYBeforeSubsub}}
	\newlength{\nprEndX} \setlength{\nprEndX}{\calc{\zposx{arrowNprInvasionsEnd}sp}}
	\newlength{\nprTurnX} \setlength{\nprTurnX}{\arrowXVerticalLeft}
...
\begin{itemize}
	\item Flip all \disputedsuccession to \marriage
	\begin{itemize}
		\item Lose \p3 per \disputedsuccession, except for \disputedsuccession on Vassals, \conj{and} your Allies with whom
		\begin{itemize}
			\item No other PR has \alliance, \conj{and}
...
\begin{itemize}
	...
	\item Flip all \marriages on your Realm
\end{itemize}
...
\begin{dynamiccontents*}{framePeaceTerms}\begin{eubox}{\fhPeaceTerms}
	\begin{multicols}{3}
		...
		\begin{itemize}
			...
			\item Remove all Loser's \alliances/\marriages
			\item Victor may discard \claim in Area(s) of gained \vassalprovinces to remove 2 \unrest from same Area(s)
		...
		\begin{itemize}
			...
			\item Loser must change State Religion and Rel. of Capital Area to State Rel. of Victor
		...
		\begin{itemize}
			...
			\item Remove all \marriage/\disputedsuccession and \alliance/\activeally, except Victor's, from the Loser's Capital
			...
			\item If Victor has \disputedsuccession on Loser's Capital, they
			\begin{itemize}
		...
		\begin{itemize}
			...
			\item May agree on any other Peace Term, except \emph{Vassalization} and \emph{Full Annexation}, but other requirements must be met
...
\begin{dynamiccontents*}{frameArrowsPageTwo}
	...
	\newlength{\peaceTermsStartYContinue} \setlength{\peaceTermsStartYContinue}{\calc{\margin + \textheight + .5\columnsep}}
	...
	\newlength{\peaceTermsTurnXTwo} \setlength{\peaceTermsTurnXTwo}{\arrowXVerticalLeft}

	\drawArrow{(0mm, \peaceTermsStartYContinue) -- (\peaceTermsTurnXTwo,\peaceTermsStartYContinue) -- (\peaceTermsTurnXTwo,\peaceTermsEndY) -- (\peaceTermsEndX,\peaceTermsEndY)}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item From DNPRs with Base Tax ≥ 10\ducats
	\end{itemize}
...
\begin{itemize}
	...
	\begin{itemize}
		\item If Age III/IV, add new Ideas to deck
	\end{itemize}
	\item PR with least \prestige (no ties) may choose replacement for 1 new Milestone or Idea
	\item PRs with 1+ \marriage score \p1
	\item PRs, \botrule{except Bots (p.~3)}, in Turn order
	\begin{itemize}
		\item Remove 1\marriage (prefer NPR), \conj{or} pay 2\diplopower
	\end{itemize}
