...
\newlength{\fhFirstPage} \setlength\fhFirstPage{\calc{\textheight - \fhPeaceSeq - \frameToTextSpacing}}
...
% Text Columns
\usepackage{flowfram}
\newflowframe[1]{\columnwidth}{\fhFirstPage}{\middleColX}{\calc{\textheight - \fhFirstPage}}[firstPageMiddleCol]
\newflowframe[1]{\columnwidth}{\fhFirstPage}{\rightColX}{\calc{\textheight - \fhFirstPage}}[firstPageRightCol]
...
\newdynamicframe[1]{\columnwidth}{\fhNprInvasions}{0mm}{\calc{\textheight - \fhNprInvasions}}[frameNprInvasions]
...
\begin{dynamiccontents*}{framePeaceSeq}\begin{eubox}{\fhPeaceSeq}
	\begin{multicols}{3}
		...
		\begin{itemize}
			...
			\item \strong{Requirements}
			\begin{itemize}
				\item Must Occupy all single Enemy's \emph{de jure} Provinces, \conj{and}
		...
		\begin{itemize}
			\item Raise \authority by 1 (p.~44-45)
			\begin{itemize}
				\item if PR Emp. wins against ext. Aggressor \conj{and} no HRE Prov. is ceded
				\item for each Area joining HRE
			\end{itemize}
			\item Drop \authority by 1 (p.~44-45)
			\begin{itemize}
				\item if PR Emp. loses against ext. Aggressor
				\item for each Area leaving HRE
			\end{itemize}
			...
			\item PRs at Peace flip their War/Truce tokens
			{\botrules
...
\begin{itemize}
	...
	\botrule{\item Bots ignore this Phase (p.~2)}
\end{itemize}
\subsubsection*{A. Reveal Events}
\begin{itemize}
	\item Draw 1 + \# PRs events (first 3 face-up)
\end{itemize}
...
\subsubsection*{\zsavepos{arrowPeaceSeqStart}D. Peace Resolutions}
...
\begin{dynamiccontents*}{frameArrowsPageOne}
	...
	% Peace Resolution arrows
	\newlength{\peaceStartY} \setlength{\peaceStartY}{\calc{\zposy{arrowPeaceSeqStart}sp + \arrowAdjustYBeforeSubsub - 5pt}}
	\newlength{\peaceStartX} \setlength{\peaceStartX}{\zposx{arrowPeaceSeqStart}sp}
	...
	\newlength{\peaceTurnX} \setlength{\peaceTurnX}{\calc{\peaceStartX + \arrowAdjustXTurnBeforeStart}}
	\newlength{\peaceTurnY} \setlength{\peaceTurnY}{\calc{\peaceSeqEndY + 5.2mm}}
	\newlength{\peaceSeqTurnX} \setlength{\peaceSeqTurnX}{\calc{\peaceSeqEndX + \arrowAdjustXTurnBeforeEnd}}

	\drawArrow{(\peaceStartX, \peaceStartY) -- (\peaceTurnX,\peaceStartY) -- (\peaceTurnX,\peaceTurnY) -- (\peaceSeqTurnX,\peaceTurnY) -- (\peaceSeqTurnX,\peaceSeqEndY) -- (\peaceSeqEndX,\peaceSeqEndY)}
	\drawLine{(\peaceStartX, \peaceStartY) -- (\peaceTurnX,\peaceStartY) -- (\peaceTurnX,\peaceTurnY) -- (\pagewidth,\peaceTurnY)} % continues in next page to Peace Terms
	...
	\newlength{\nprEndX} \setlength{\nprEndX}{\zposx{arrowNprInvasionsEnd}sp}
	\newlength{\nprTurnX} \setlength{\nprTurnX}{\calc{\nprStartX + \arrowAdjustXTurnBeforeStart}}
	\newlength{\nprTurnY} \setlength{\nprTurnY}{\calc{\nprEndY + 6.5mm}}
	\newlength{\nprTurnXTwo} \setlength{\nprTurnXTwo}{\calc{\nprEndX + \arrowAdjustXTurnBeforeEnd}}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item If \conj{any} Area contains your \vassals
		\begin{itemize}
...
\begin{dynamiccontents*}{framePeaceTerms}\begin{eubox}{\fhPeaceTerms}
	\begin{multicols}{3}
		...
		\begin{itemize}
			...
			\item Remove all Loser's \alliances/\marriages
			\item If Loser is Active Ally, then lose \p2
...
\begin{dynamiccontents*}{frameArrowsPageTwo}
	...
	% Peace Terms arrow
	\newlength{\peaceTurnYContinue} \setlength{\peaceTurnYContinue}{\calc{\zposy{arrowPeaceSeqEnd}sp + \arrowAdjustYBeforeSubsub + 5.2mm}}
	...
	\newlength{\peaceTermsTurnX} \setlength{\peaceTermsTurnX}{\calc{\peaceTermsEndX + \arrowAdjustXTurnBeforeEnd}}

	\drawArrow{(0mm, \peaceTurnYContinue) -- (\peaceTermsTurnX,\peaceTurnYContinue) -- (\peaceTermsTurnX,\peaceTermsEndY) -- (\peaceTermsEndX,\peaceTermsEndY)}
...
\begin{itemize}
	...
	\item \botrule{Discard Bot \colonists if there is no vacant Territory nor own Colonial \claim (p.~3)}
...
\begin{dynamiccontents*}{frameArrowsPageThree}
	...
	\begin{tikzpicture}
		...
		% Manpower arrow
		\newlength{\manpowerStartY} \setlength{\manpowerStartY}{\calc{\zposy{arrowManpowerStart}sp + \arrowAdjustYText}}
		...
		\newlength{\manpowerTurnX} \setlength{\manpowerTurnX}{\calc{\manpowerStartX + \arrowAdjustXTurnBeforeStart}}
		...
		\newlength{\impinfStartY} \setlength{\impinfStartY}{\calc{\zposy{arrowImpInfluenceStart}sp + \arrowAdjustYText}}
		...
		\newlength{\impinfTurnX} \setlength{\impinfTurnX}{\calc{\impinfStartX + \arrowAdjustXTurnBeforeStart}}

		\drawArrow{(\impinfStartX, \impinfStartY) -- (\impinfTurnX,\impinfStartY) -- (\impinfTurnX,\impinfEndY) -- (\impinfEndX,\impinfEndY)}

		% HRE Religion arrow
		\newlength{\hreStartY} \setlength{\hreStartY}{\calc{\zposy{arrowHreReligionStart}sp + \arrowAdjustYText}}
		...
		\newlength{\hreTurnX} \setlength{\hreTurnX}{\calc{\hreStartX + \arrowAdjustXTurnBeforeStart}}

		\drawArrow{(\hreStartX, \hreStartY) -- (\hreTurnX,\hreStartY) -- (\hreTurnX,\hreEndY) -- (\hreEndX,\hreEndY)}

		% Bot Deck arrow
		\newlength{\botDeckStartY} \setlength{\botDeckStartY}{\calc{\zposy{arrowBotDeckStart}sp + \arrowAdjustYText}}
		...
		\newlength{\botDeckTurnX} \setlength{\botDeckTurnX}{\calc{\botDeckStartX + \arrowAdjustXTurnBeforeStart}}
		...
		\newlength{\crusadeTurnX} \setlength{\crusadeTurnX}{\calc{\crusadeEndX + \arrowAdjustXTurnBeforeEnd - 1mm}}
		...
		\newlength{\powerTurnX} \setlength{\powerTurnX}{\calc{\powerEndX + \arrowAdjustXTurnBeforeEnd}}
